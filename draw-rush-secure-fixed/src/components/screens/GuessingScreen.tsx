...
export function GuessingScreen() {
  ...
  const submit = async () => {
    ...
    if (!text || !isMyTurn || state.isLoading || state.authStatus !== 'authenticated') return;
    try {
  ...
  const judge = async (guessId: string, correct: boolean) => {
    if (state.isLoading || state.authStatus !== 'authenticated') return;
    try {
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <GlassPanel glow="cyan" className="p-6">
              ...
              <div className="flex gap-3">
                ...
                <NeonButton variant="cyan" onClick={() => { void submit(); }} disabled={!guess.trim() || state.isLoading || state.authStatus !== 'authenticated'}><Send className="w-5 h-5" /></NeonButton>
              </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            ...
            {pendingToJudge.map((item) => (
              <GlassPanel key={item.id} className="p-4 flex flex-wrap items-center gap-3">
                ...
                <NeonButton size="sm" variant="cyan" disabled={state.isLoading || state.authStatus !== 'authenticated'} onClick={() => { void judge(item.id, true); }} icon={<CheckCircle2 className="w-4 h-4" />}>صحيحة</NeonButton>
                <NeonButton size="sm" variant="outline" disabled={state.isLoading || state.authStatus !== 'authenticated'} onClick={() => { void judge(item.id, false); }} icon={<XCircle className="w-4 h-4" />}>خاطئة</NeonButton>
              </GlassPanel>
            ))}
          </motion.div>
        )}
