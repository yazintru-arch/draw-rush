    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 pt-20 pb-8">
      <div className="max-w-4xl w-full flex flex-col gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex items-center justify-center gap-4">
          <NeonButton variant={self?.isReady ? 'outline' : 'cyan'} disabled={!self || state.isLoading} onClick={() => { void toggleReady().catch(() => undefined); }} icon={self?.isReady ? <Check className="w-5 h-5" /> : <UserPlus className="w-5 h-5" />}>
            {self?.isReady ? 'إلغاء الجاهزية' : 'أنا جاهز!'}
          </NeonButton>
          <NeonButton variant="purple" disabled={!server.room.isHost || !allReady || state.isLoading} onClick={() => { void beginGame().catch(() => undefined); }} icon={<Play className="w-5 h-5" />}>
            ابدأ اللعبة
          </NeonButton>
        </motion.div>
