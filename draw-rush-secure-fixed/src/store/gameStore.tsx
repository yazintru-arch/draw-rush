...
type GameAction =
  ...
  | { type: 'SET_AUTH_STATUS'; status: GameState['authStatus'] }
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'NEXT_ROUND':
      return state;
    case 'RESET_GAME':
      return { ...initialState, authStatus: 'authenticated', isLoading: false };
    default:
      return state;
  }
}
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_AUTH_STATUS', status: 'error' });
  ...
  const notifyAndRefresh = useCallback(async (operation: () => Promise<ServerGameState | void>) => {
    ...
    try {
      // This explicit provider boundary prevents a first user click from
      // issuing create_room/join_room before Auth has restored or created a
      // guest session. Each API method repeats the same guard as defense in depth.
      await ensureGuestSession();
      dispatch({ type: 'SET_AUTH_STATUS', status: 'authenticated' });
    } catch (error) {
      dispatch({ type: 'SET_AUTH_STATUS', status: 'error' });
      dispatch({ type: 'SET_ERROR', error: error instanceof Error ? error.message : 'تعذر تهيئة جلسة الضيف.' });
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      dispatch({ type: 'SET_LOADING', loading: true });
      dispatch({ type: 'SET_AUTH_STATUS', status: 'initializing' });
      try {
        await ensureGuestSession();
        dispatch({ type: 'SET_AUTH_STATUS', status: 'authenticated' });
  ...
  useEffect(() => {
    ...
    void subscribeToRoom(roomCode, () => { void refresh(); })
      .then((subscribedChannel) => {
        if (!isActive) {
          void subscribedChannel.unsubscribe();
          return;
        }
      ...
      .catch((error: unknown) => {
        if (!isActive) return;
        dispatch({ type: 'SET_ERROR', error: error instanceof Error ? error.message : 'تعذر الاتصال بقناة التحديث.' });
      });
