...
import { requireAuthenticatedSession, requireAuthenticatedSupabase } from './supabase';
...
async function rpcState(name: string, parameters: JsonRecord): Promise<ServerGameState> {
  // No protected RPC is allowed to race the asynchronous Auth restore/sign-in.
...
export async function subscribeToRoom(roomCode: string, onChange: () => void): Promise<RealtimeChannel> {
  const client = await requireAuthenticatedSupabase();
  return client
